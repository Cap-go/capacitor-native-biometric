...
This is a plugin specific list of error codes that can be thrown on `verifyIdentity()` failure, set as part of `isAvailable()`, or returned from `getSecureCredentials()` when no protected credentials exist for the requested server. It consolidates Android and iOS specific authentication error codes into one combined error list.
...
| 21   | No Protected Credentials Found   | Android, iOS                 |

**Code 21 (`getSecureCredentials`)** — Stable, expected rejection when no biometric-protected credentials are stored for the server (for example after `deleteCredentials()`, a fresh install, or credentials invalidated by biometric enrollment changes). Platform messages differ slightly:

- **Android:** `No protected credentials found`
- **iOS:** `No protected credentials found for server`

Use `isCredentialsSaved()` to avoid the call when nothing is stored, or handle code `21` in your catch block. See [`getSecureCredentials()`](#getsecurecredentials) for details.
...
were deleted, or were invalidated after biometric enrollment changes. Use
`isCredentialsSaved()` to check first, or handle the rejection in your app flow.
...
| **`NO_PROTECTED_CREDENTIALS_FOUND`** | <code>21</code> | No biometric-protected credentials exist for the requested server. Platform: Android, iOS (`getSecureCredentials()`). Thrown when `getSecureCredentials()` is called but nothing was stored with `accessControl` BIOMETRY_CURRENT_SET or BIOMETRY_ANY for that server — for example after `deleteCredentials()`, a fresh install, or credentials removed from the secure enclave after biometric enrollment changes. Rejection message differs by platform: - Android: `No protected credentials found` - iOS: `No protected credentials found for server` | 8.4.0 |
